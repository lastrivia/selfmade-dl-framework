...
/**  normal:  [n, c_in, ., .] * [c_out, c_in, ., .] -> [n, c_out, ., .] (forward; input grad)
  ...
  */
void cpu_kernel_conv_fp32(const size_t samples, const size_t c_in, const size_t c_out,
            ...
            for (size_t j = 0; j < h_out; j++) {
                for (size_t k = 0; k < w_out; k++) {
                    ...
                    for (size_t ci = 0; ci < c_in; ci++) {
                        for (
                            size_t j_kernel = static_cast<size_t>(std::max(static_cast<ssize_t>(0),
                                                                           static_cast<ssize_t>(h_padding) - static_cast<ssize_t>(j)));
                            j_kernel < std::min(h_kernel, h_in + h_padding - j);
                            j_kernel++
                        ) {
                            for (
                                size_t k_kernel = static_cast<size_t>(std::max(static_cast<ssize_t>(0),
                                                                               static_cast<ssize_t>(w_padding) - static_cast<ssize_t>(k)));
                                k_kernel < std::min(w_kernel, w_in + w_padding - k);
                                k_kernel++
                            ) {
