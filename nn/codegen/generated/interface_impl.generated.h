...
inline tensor tensor::operator-(const tensor &b) const {
    ...
    case data_type::fp32: {
        ...
        if ((a->requires_grad_ || b->requires_grad_) && !global_no_grad) {
            result->requires_grad_ = true;
            result->grad_node_ = new grad_node_sub_fp32(result, a, b);
        }
...
inline tensor tensor::operator*(const tensor &b) const {
    ...
    case data_type::fp32: {
        ...
        if ((a->requires_grad_ || b->requires_grad_) && !global_no_grad) {
            result->requires_grad_ = true;
            result->grad_node_ = new grad_node_mul_fp32(result, a, b);
        }
...
inline tensor tensor::operator/(const tensor &b) const {
    ...
    case data_type::fp32: {
        ...
        if ((a->requires_grad_ || b->requires_grad_) && !global_no_grad) {
            result->requires_grad_ = true;
            result->grad_node_ = new grad_node_div_fp32(result, a, b);
        }
...
inline tensor square(const tensor &t) {
    ...
    case data_type::fp32: {
        ...
        if ((t->requires_grad_) && !global_no_grad) {
            result->requires_grad_ = true;
            result->grad_node_ = new grad_node_square_fp32(result, t);
        }

        return result;
    }
    case data_type::int32:
        throw nn_except("tensor: square does not support data type int32", __FILE__, __LINE__);
        break;
    default:
        throw nn_except("tensor: unknown data type", __FILE__, __LINE__);
        break;
...
inline tensor sqrt(const tensor &t) {
    ...
    case data_type::fp32: {
        ...
        if ((t->requires_grad_) && !global_no_grad) {
            result->requires_grad_ = true;
            result->grad_node_ = new grad_node_sqrt_fp32(result, t);
        }

        return result;
    }
    case data_type::int32:
        throw nn_except("tensor: sqrt does not support data type int32", __FILE__, __LINE__);
        break;
    default:
        throw nn_except("tensor: unknown data type", __FILE__, __LINE__);
        break;
...
inline tensor relu(const tensor &t) {
    ...
    case data_type::fp32: {
        ...
        if ((t->requires_grad_) && !global_no_grad) {
            result->requires_grad_ = true;
            result->grad_node_ = new grad_node_relu_fp32(result, t);
        }

        return result;
    }
    case data_type::int32:
        throw nn_except("tensor: relu does not support data type int32", __FILE__, __LINE__);
        break;
    default:
        throw nn_except("tensor: unknown data type", __FILE__, __LINE__);
        break;
...
inline tensor tensor::operator+(float scalar) const {
    ...
    if (t->dtype_ != data_type::fp32)
        ...
        if ((t->requires_grad_) && !global_no_grad) {
            result->requires_grad_ = true;
            result->grad_node_ = new grad_node_add_scalar_fp32(result, t, scalar);
        }
...
inline tensor tensor::operator-(float scalar) const {
    ...
    if (t->dtype_ != data_type::fp32)
        ...
        if ((t->requires_grad_) && !global_no_grad) {
            result->requires_grad_ = true;
            result->grad_node_ = new grad_node_sub_scalar_fp32(result, t, scalar);
        }
...
inline tensor tensor::operator*(float scalar) const {
    ...
    if (t->dtype_ != data_type::fp32)
        ...
        if ((t->requires_grad_) && !global_no_grad) {
            result->requires_grad_ = true;
            result->grad_node_ = new grad_node_mul_scalar_fp32(result, t, scalar);
        }
...
inline tensor tensor::operator/(float scalar) const {
    ...
    if (t->dtype_ != data_type::fp32)
        ...
        if ((t->requires_grad_) && !global_no_grad) {
            result->requires_grad_ = true;
            result->grad_node_ = new grad_node_div_scalar_fp32(result, t, scalar);
        }
...
inline tensor pow(const tensor &t, float scalar) {
    ...
    if (t->dtype_ != data_type::fp32)
        throw nn_except("tensor: operands for pow have different data types", __FILE__, __LINE__);
    else {
        tensor result(t->shape_, t->device_, t->dtype_);
        device_desc device = result->device_;
        ...
        if ((t->requires_grad_) && !global_no_grad) {
            result->requires_grad_ = true;
            result->grad_node_ = new grad_node_pow_fp32(result, t, scalar);
        }
...
inline tensor add_broadcast(const tensor &a, const tensor &b) {
    if (a->device_ != b->device_)
        throw nn_except("tensor: operands for add_broadcast are on different devices", __FILE__, __LINE__);
    if (a->dtype_ != b->dtype_)
        throw nn_except("tensor: operands for add_broadcast have different data types", __FILE__, __LINE__);

    // [codegen] shape: broadcast
    size_t ndim = std::max({a->shape_.ndim, b->shape_.ndim});
    workspace a_mask_workspace(ndim * sizeof(bool), device_type::cpu), b_mask_workspace(ndim * sizeof(bool), device_type::cpu), ret_dims_workspace(ndim * sizeof(size_t), device_type::cpu);
    ...
    for (size_t i = 0; i < ndim; ++i) {
        ...
        if (i < b->shape_.ndim) {
            if (b_dims[i] == 1)
                ...
                throw nn_except(std::string() + "tensor: add_broadcast cannot handle tensors " + std::string(a->shape_) + " and " + std::string(b->shape_), __FILE__ , __LINE__);
        }
        else
            b_mask[i] = false;
    }
    ...
    case data_type::fp32: {
        ...
        if ((a->requires_grad_ || b->requires_grad_) && !global_no_grad) {
            result->requires_grad_ = true;
            result->grad_node_ = new grad_node_add_broadcast_fp32(result, a, b, std::move(a_mask_workspace), std::move(b_mask_workspace));
        }

        return result;
    }
    case data_type::int32:
        throw nn_except("tensor: add_broadcast does not support data type int32", __FILE__, __LINE__);
        break;
    default:
        throw nn_except("tensor: unknown data type", __FILE__, __LINE__);
        break;
...
inline tensor sum(const tensor &t, std::vector<size_t> dims) {
    ...
    case data_type::fp32: {
        ...
        if ((t->requires_grad_) && !global_no_grad) {
            throw nn_except("tensor: sum does not have a corresponding autograd node", __FILE__, __LINE__);
        }

        return result;
    }
    case data_type::int32:
        throw nn_except("tensor: sum does not support data type int32", __FILE__, __LINE__);
        break;
    default:
        throw nn_except("tensor: unknown data type", __FILE__, __LINE__);
        break;
...
inline tensor cross_entropy(const tensor &logits, const tensor &label) {
    if (logits->device_ != label->device_)
        throw nn_except("tensor: operands for cross_entropy are on different devices", __FILE__, __LINE__);
    if (logits->dtype_ != label->dtype_)
        throw nn_except("tensor: operands for cross_entropy have different data types", __FILE__, __LINE__);

    // [codegen] shape: ewise
    if (logits->shape_ != label->shape_)
        throw nn_except(std::string() + "tensor: cross_entropy cannot handle tensors " + std::string(logits->shape_) + " and " + std::string(label->shape_), __FILE__ , __LINE__);
    ...
    case data_type::fp32: {
        ...
        if ((logits->requires_grad_ || label->requires_grad_) && !global_no_grad) {
            result->requires_grad_ = true;
            result->grad_node_ = new grad_node_cross_entropy_fp32(result, logits, label);
        }

        return result;
    }
    case data_type::int32:
        throw nn_except("tensor: cross_entropy does not support data type int32", __FILE__, __LINE__);
        break;
    default:
        throw nn_except("tensor: unknown data type", __FILE__, __LINE__);
        break;
...
inline tensor maxpool(const tensor &t, size_t h_stride, size_t w_stride) {
    ...
    case data_type::fp32: {
        ...
        if ((t->requires_grad_) && !global_no_grad) {
            result->requires_grad_ = true;
            result->grad_node_ = new grad_node_maxpool_fp32(result, t, h_stride, w_stride, std::move(mask_workspace));
        }

        return result;
    }
    case data_type::int32:
        throw nn_except("tensor: maxpool does not support data type int32", __FILE__, __LINE__);
        break;
    default:
        throw nn_except("tensor: unknown data type", __FILE__, __LINE__);
        break;
...
tensor matmul(const tensor &a, const tensor &b) {
    if (a->device_ != b->device_)
        throw nn_except("tensor: operands for matmul are on different devices", __FILE__, __LINE__);
    if (a->dtype_ != b->dtype_)
        throw nn_except("tensor: operands for matmul have different data types", __FILE__, __LINE__);

    // [codegen] shape: matmul
    if (a->shape_.ndim != 2)
        throw nn_except(std::string() + "tensor: matmul cannot handle tensor a " + std::string(a->shape_)
                        + " with " + std::to_string(a->shape_.ndim) + " dimensions (2 expected)", __FILE__, __LINE__);
    if (b->shape_.ndim != 2)
        throw nn_except(std::string() + "tensor: matmul cannot handle tensor b " + std::string(b->shape_)
    ...
    if (matmul_k != (transpose_b ? b->shape_.lengths[0] : b->shape_.lengths[1]))
        throw nn_except(std::string() + "tensor: incompatible shapes " + std::string(a->shape_) + (transpose_a ? "^T" : "")
    ...
    case data_type::fp32: {
        ...
        if ((a->requires_grad_ || b->requires_grad_) && !global_no_grad) {
            result->requires_grad_ = true;
            result->grad_node_ = new grad_node_matmul_fp32(result, a, b, transpose_a, transpose_b, matmul_m, matmul_n, matmul_k);
        }

        return result;
    }
    case data_type::int32:
        throw nn_except("tensor: matmul does not support data type int32", __FILE__, __LINE__);
        break;
    default:
        throw nn_except("tensor: unknown data type", __FILE__, __LINE__);
        break;
...
inline tensor conv(const tensor &input, const tensor &kernel, const tensor &bias, size_t h_padding, size_t w_padding) {
    if (input->device_ != kernel->device_ || input->device_ != bias->device_)
        throw nn_except("tensor: operands for conv are on different devices", __FILE__, __LINE__);
    if (input->dtype_ != kernel->dtype_ || input->dtype_ != bias->dtype_)
        throw nn_except("tensor: operands for conv have different data types", __FILE__, __LINE__);

    // [codegen] shape: conv
    if (input->shape_.ndim != 4)
        throw nn_except(std::string() + "tensor: conv cannot handle tensor input " + std::string(input->shape_)
                        + " with " + std::to_string(input->shape_.ndim) + " dimensions (4 expected)", __FILE__, __LINE__);
    if (kernel->shape_.ndim != 4)
        throw nn_except(std::string() + "tensor: conv cannot handle tensor kernel " + std::string(kernel->shape_)
    ...
    if (kernel->shape_.lengths[2] != conv_ci)
        throw nn_except(std::string() + "tensor: convolution channel mismatch between input " + std::string(input->shape_)
                        + " and kernel " + std::string(kernel->shape_), __FILE__, __LINE__);
    if (bias->shape_.ndim != 4)
        throw nn_except(std::string() + "tensor: conv cannot handle tensor bias " + std::string(bias->shape_)
    ...
    if (bias->shape_.lengths[2] != conv_co || bias->shape_.size != bias->shape_.lengths[2])
        throw nn_except(std::string() + "tensor: incompatible bias shape " + std::string(bias->shape_) + " with output channels "
    ...
    case data_type::fp32: {
        ...
        if ((input->requires_grad_ || kernel->requires_grad_ || bias->requires_grad_) && !global_no_grad) {
            result->requires_grad_ = true;
            result->grad_node_ = new grad_node_conv_fp32(result, input, kernel, bias, h_padding, w_padding, conv_n, conv_ci, conv_co, conv_h_out, conv_w_out);
        }

        return result;
    }
    case data_type::int32:
        throw nn_except("tensor: conv does not support data type int32", __FILE__, __LINE__);
        break;
    default:
        throw nn_except("tensor: unknown data type", __FILE__, __LINE__);
        break;
